...
class EcosystemPolicyTests(unittest.TestCase):
    def test_policy_contract_is_present(self):
        ...
        self.assertIn("Policy-Version: 1.19.1", agents)
        self.assertIn("Policy-Version: 1.19.1", policy)
        self.assertIn("Responsive embedded view contract", policy)
        self.assertIn("visualViewport", policy)
        self.assertIn("flow card", policy)
        self.assertIn("Per-device notification language", policy)
        ...
        self.assertIn("native visual dashboard editor", policy)
        self.assertIn("Unknown HA versions fail safely", policy)
        self.assertIn("installation/system language", policy)
        self.assertIn("English and German together", policy)
