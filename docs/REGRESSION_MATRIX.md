...
| Silent planner control flow | Presence wait, Vacation, skip, postpone, unavailable robot and unmet mop guards remain visible in planner entities/Card/Badge but never emit technical `not started` pushes | `tests/test_notifications.py`, runtime E2E |
...
| Completion lifecycle | A resumable Valetudo mop-wash visit remains `dock_service`; only confirmed final docking sends one routed completion message with available time/area metrics and keeps native `completed` visible before returning to planning | `tests/test_models.py`, `tests/test_notifications.py`, runtime E2E |
| Robot error notification | Vacuum state and detailed adapter error updates are debounced into one robot-specific notification; a Valetudo error cannot also emit a duplicate dock-maintenance message | `tests/test_notifications.py`, runtime E2E |
...
| Robot display names | Setup and Options persist a per-vacuum display name; legacy generic suffixes receive a concise fallback; completion titles, Card and Badge share the same status mapping and technical starter mission names remain details only | `tests/test_notifications.py`, `tests/test_card_runtime.js`, runtime E2E |
